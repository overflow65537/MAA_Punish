...
- **框架层（动作与通用能力）**：`agent/action/tool/CombatActions.py`
- **信息层（角色配置与素材索引）**：`agent/action/tool/LoadSetting.py`（`ROLE_ACTIONS`）
- **业务层（每个角色/通用战斗逻辑）**：`agent/action/exclusives/*.py`

除此之外，还有一个**必须的注册入口**：`agent/agent_file.py`。所有要在 Pipeline 里通过 `custom_action` 调用到的类，都需要在这里注册。
...
- **Agent 注册解析**：`agent/main.py` 启动 Agent，导入 `agent_file.py`，将 `"XXX"` 映射到你实现的 `CustomAction` 子类
...
- **角色配置（信息层）**：`agent/action/tool/LoadSetting.py`
  - `ROLE_ACTIONS`：角色名 → `cls_name`（业务类名）/ 模板 / 元信息
- **通用战斗对象（框架层）**：`agent/action/tool/CombatActions.py`
  - 封装了攻击、闪避、技能、消球、QTE、状态识别、信号球决策等
- **角色脚本（业务层）**：`agent/action/exclusives/*.py`
  - 每个文件一般对应一个 `cls_name`
- **CustomAction 注册表（必须）**：`agent/agent_file.py`
...
在 `agent/action/tool/LoadSetting.py` 的 `ROLE_ACTIONS` 中新增一项。
...
- **文件路径**：`agent/action/exclusives/<cls_name>.py`
...
在 `agent/agent_file.py` 增加（并确保有 import）：
...
说明：当前 `agent/action/exclusives/` 里**显式调用** `Switch()` 的脚本主要是以下两类用法（其余角色脚本更多是 `auto_qte()`/`trigger_qte()`，未直接调用 `Switch()`）。
...
- **忘了注册**：没在 `agent/agent_file.py` 添加 `@AgentServer.custom_action("YourRole")`
...
- `agent/action/exclusives/GeneralFight.py`：通用战斗最小示例
- `agent/action/exclusives/Oblivion.py`：状态节点分支（`check_status()`）的典型写法
- `agent/action/exclusives/Shukra.py`：信号球决策 + 限时循环的典型写法
