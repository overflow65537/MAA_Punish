...
S3_ball,S4_ball,]
    ...
    def find_max_elimination(self, board):
        ...
        if best_skill_move and (not normal_move or best_skill_move[2] >= normal_eliminated):
    ...
    def count_eliminations(self, board):
        ...
        for i, j in list(to_remove):
            ...
            if val in [5, 6, 7, 8]:  # 技能球
                if val == 5:  # 随机消除4个球
                    to_remove.update({(random.randint(0, 7), random.randint(0, 7)) for _ in range(4)})
                elif val == 6:  # 3x3区域(9个格子)
                    to_remove.update({(i + dx, j + dy) for dx in [-1,0,1] for dy in [-1,0,1] 
                                    if 0 <= i + dx < 8 and 0 <= j + dy < 8})
                elif val == 7:  # 整列(8个格子)
                    to_remove.update({(x, j) for x in range(8)})
                elif val == 8:  # 随机消除12个球
                    to_remove.update({(random.randint(0, 7), random.randint(0, 7)) for _ in range(12)})
