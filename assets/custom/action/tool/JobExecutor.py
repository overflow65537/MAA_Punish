...
class JobExecutor:
    ...
    def __init__(self, 
                 job_factory: Callable[[], Union[Job, JobWithResult]],
                 status_attr: str,
                 action_name: str,
                 success_attr: str = "succeeded"):
        """
        ...
        self._status_check_attr = status_attr
        self._success_check_attr = success_attr
        self._current_job = None  # 当前监控的Job实例
        
    def _validate_attributes(self, job: Union[Job, JobWithResult]):
        """属性验证(确保必要属性存在)"""
        for attr in [self._status_check_attr, self._success_check_attr]:
            if not hasattr(job, attr):
                raise ValueError(f"Job实例缺少必要属性: {attr}")
    ...
    def execute(self, 
        ...
        for attempt in range(1, max_retries + 1):
            ...
            self._validate_attributes(self._current_job)
    ...
    def _log_success(self, verbose: bool):
        ...
        if not verbose:
            return
        # 直接使用Status的字符串表示
        status = str(self._current_job.status)
    ...
    def _log_error(self, error: Exception, verbose: bool):
        """异常日志"""
        if verbose:
            status = str(self._current_job.status) if self._current_job else "无有效任务"
...
#     status_attr="done",
#     action_name="模拟任务",
#     success_attr="succeeded"
# )

# 执行任务
# success = executor.execute(timeout=3, max_retries=2, verbose=True)